...
def imgSegment(filePath,fileType,matchFlag,outputCoordinateFlag, saveFolder,adaptiveThreshold,outputBigImg ,morphFlag ,
               noSuperviewThre,sigmaBlur, kernalSizeBlur ,binary_threshold ,saveWithBackgroundFlag,sizeLimit, paddingNum, adaptPare1,adaptPare2,morphPara1):
    ...
    for n in np.arange(imgNum):
        ...
        imgBlur=cv.GaussianBlur(img,kernalSizeBlur,cv.BORDER_DEFAULT)
    # ----------------------way 1 no supervise, the threshold num is calculated by the isodata method
        if noSuperviewThre:
            ...
            mask=np.uint8(imgBlur>binary_threshold)
